...
// установка секунд таймера
  ...
  playStart = (idx) => {
    ...
    const item = this.state.tasks[idx]
    const id = this.state.tasks.indexOf(item)
    

    
    this.setState((state) => {
      const oldItem = state.tasks[idx];
      const newItem = { ...oldItem, timerIsOn: true, pause: oldItem.pause ? !oldItem.pause : oldItem.pause };
  ...
  pauseStart = (idx) => {
    ...
    // this.setState((state) => {
    //   const oldItem = state.tasks[idx];
    //   const newItem = { ...oldItem, timerIsOn: false, pause: !oldItem.pause, play: !oldItem.play };
  ...
  tick = (idx) => {
    ...
    const item = this.state.tasks[idx]
    const id = this.state.tasks.indexOf(item)
    const {timerIsOn} = item
    const timerId = item.timer
    ...
    this.timer = setInterval(() => {
      ...
      if (timeSeconds === 0 && timeMinutes > 0) { 
        ...
        this.state.tasks[idx].timer = this.timer
      }
